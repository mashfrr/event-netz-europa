...
async function migrateFinal() {
  try {
    console.log('🚀 Starting FINAL migration with proper data types...');
    console.log('📋 This will migrate all events with correct data formatting');
    ...
    for (const item of items) {
      try {
        ...
        // Map category to array format
        const mondayCategory = getColumnValue('dropdown_mkvxvvwz');
        let categories = ['community']; // Default as array
        if (mondayCategory) {
          ...
          } else if (categoryLower.includes('erasmus')) {
            categories = ['education', 'erasmus'];
          }
        }

        // Map to the CORRECT schema with proper data types
        const eventData = {
          title: item.name,
        ...
        if (error) {
          ...
          console.log(`✅ Synced event: "${item.name}" (Categories: ${categories.join(', ')})`);
          successCount++;
        }
      } catch (itemError) {
    ...
    console.log('- Categories mapped to array format');
