...
async function migrateCorrectDates() {
  try {
    console.log('🚀 Starting CORRECTED migration with proper date mapping...');
    console.log('📋 This will fix the date mapping and add registration deadline');
    ...
    for (const item of items) {
      try {
        ...
        // - registration_deadline: datum (registration deadline)
        ...
        const eventData = {
          ...
          registration_deadline: getColumnValue('datum') || null, // Registration deadline
        ...
        if (error) {
          ...
          console.log(`   📅 End: ${eventData.end_date || 'null'}`);
          console.log(`   ⏰ Registration Deadline: ${eventData.registration_deadline || 'null'}`);
          successCount++;
        }
      } catch (itemError) {
    ...
    console.log('\n📋 DATE MAPPING CORRECTED:');
    console.log('- start_date: Event start date (date_mkvxvryw)');
    console.log('- end_date: Event end date (date_mkvx3kj3)');
    console.log('- registration_deadline: Registration deadline (datum)');
...
migrateCorrectDates().catch(console.error);
