...
async function syncMondayToSupabase() {
  try {
    ...
    console.log('📋 Using working version without registration_deadline column');
    ...
    for (const item of items) {
      try {
        ...
        // WORKING VERSION - without registration_deadline column
        const eventData = {
          title: item.name,
          ...
          picture: null
          // Note: registration_deadline (Anmeldefrist) will be added when column exists
        };
        ...
        if (existingEvent) {
          ...
          if (error) {
            ...
            console.log(`   ⏰ Anmeldefrist: ${getColumnValue('datum') || 'null'} (will be added when column exists)`);
            updatedCount++;
          }
        } else {
          // Create new event
    ...
    if (errorCount === 0) {
      ...
      console.log('📋 Note: Anmeldefrist (registration deadline) will be added when column is created');
    }
