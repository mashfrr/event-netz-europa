...
async function syncMondayToSupabase() {
  try {
    ...
    console.log('📋 Including status, registration_deadline, and all other mappings');
    ...
    for (const item of items) {
      try {
        ...
        // COMPLETE MAPPING - including status and registration_deadline
        const eventData = {
          title: item.name,
    ...
    if (errorCount === 0) {
      ...
      console.log('🎯 Status, Anmeldefrist, and all other data properly mapped!');
    } else {
...
console.log('🎯 Status, Anmeldefrist, and all mappings included');
