...
async function testGermanStatus() {
  try {
    console.log('🧪 Testing German status mapping...');
    console.log('🇩🇪 This will show how German status values are preserved\n');
    ...
    for (const item of items) {
      try {
        ...
        // PRESERVE ORIGINAL GERMAN STATUS VALUES
        const mondayStatus = getColumnValue('status');
        let status = mondayStatus || 'Entwurf'; // Default to Entwurf if no status
        
        // Keep the original German values: Entwurf, Abgelehnt, Genehmigt
        if (mondayStatus) {
          status = mondayStatus; // Keep original German status
        }

        // COMPLETE MAPPING - including German status values
        const eventData = {
          title: item.name,
          ...
          status: status, // German status: Entwurf, Abgelehnt, Genehmigt
        ...
        if (existingEvent) {
          ...
          if (error) {
            ...
            console.log(`   📊 Status: ${status} (German original preserved)`);
    ...
    console.log(`\n🎉 German status test completed!`);
    console.log(`✅ Updated: ${updatedCount} events`);
    console.log(`❌ Errors: ${errorCount} events`);
    
    if (errorCount === 0) {
      console.log('\n🎊 SUCCESS! German status values preserved!');
      console.log('🇩🇪 Status mapping:');
      console.log('   - Entwurf (Draft)');
      console.log('   - Abgelehnt (Rejected)');
      console.log('   - Genehmigt (Approved)');
      console.log('\n📋 All events now have:');
      console.log('   - Correct event start/end dates');
      console.log('   - Registration deadlines (Anmeldefrist)');
      console.log('   - German status values preserved');
      console.log('   - All other event information');
    }
    
  } catch (error) {
    console.error('💥 Fatal error during German status test:', error);
  }
}

// Run German status test
testGermanStatus().catch(console.error);
